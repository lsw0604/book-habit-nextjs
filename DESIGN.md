...
> book-habit-nextjs는 **독서 기록·스트릭 관리 + 목표/통계 + 서평·노트 + 소셜/공유**를 아우르는 독서 습관 앱입니다. 타깃은 가벼운 독서 입문자와 다독가를 모두 아우르는 범용 사용자층이며, 톤은 Notion·Linear·Todoist 계열의 미니멀 생산성 도구를 참고합니다. 컬러는 base 토큰(원시값) → light/dark 시맨틱 토큰(역할값)으로 참조하는 디자인 토큰 구조를 사용하며, 라이트/다크 모두 확정되어 있습니다. 타이포그래피·간격·elevation 세부값은 업계 표준(4/8px spacing, 4단계 elevation)으로 채웠으니 별도 조정이 필요하면 알려주세요.
